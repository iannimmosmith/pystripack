...
!
!    Input, double precision X(K), Y(K), Z(K), the coordinates of the nodes.
!
...
!  Parameters:
!
!    Input, double precision V1(3), V2(3), V3(3), the Cartesian coordinates of unit 
!    vectors (the three triangle vertices in any order).  These vectors, 
!    if nonzero, are implicitly scaled to have length 1.
!
!    Output, double precision AREAS, the area of the spherical triangle defined by
...
!
  areas = dble ( a1 + a2 + a3 - acos ( -1.0D+00 ) )
...
!  Parameters:
!
!    Input, double precision V1(3), V2(3), V3(3), the coordinates of the three triangle
!    vertices (unit vectors) in counter clockwise order.
!
!    Output, double precision C(3), the coordinates of the circumcenter unless
...
!
!    Output, double precision XC(2*N-4), YC(2*N-4), ZC(2*N-4), the coordinates
...
!    if NB > 0.
!
!    Output, double precision RC(2*N-4), the circumradii (the arc lengths or
...
subroutine d3vec_normalize ( n, x, y, z )
!
!*******************************************************************************
!
!! D3VEC_NORMALIZE normalizes a list of 3D vectors to have unit Euclidean norm.
!
!
!  Modified:
!
...
!
!    Input/output, double precision X(N), Y(N), Z(N), the coordinates of the nodes in the
...
!
  implicit none
!
  double precision dp12
  double precision dp1l
  double precision dp1r
  double precision dp2l
  double precision dp2r
  integer i
  integer ier
  integer ierr
  integer in1
...
!
      if ( left ( x1, y1, z1, x2, y2, z2, x(nl), y(nl), z(nl) ) ) then
        ...
        if ( (dp2l-dp12*dp1l >= 0.0D+00 .or. dp2r-dp12*dp1r >= 0.0D+00 )  .and. &
             (dp1l-dp12*dp2l >= 0.0D+00 .or. dp1r-dp12*dp2r >= 0.0D+00 ) ) then
          go to 6
        end if
!
...
!
!    Output, double precision DF, value of an increasing function (negative cosine) of 
...
!  Parameters:
!
!    Input, double precision P(3), the coordinates of the point (unit vector) to be
!    located.
!
!    Input, integer LV, the length of arrays XV, YV, and ZV.
!
!    Input, double precision XV(LV), YV(LV), ZV(LV), the coordinates of unit vectors 
...
!
  ...
  double precision d
  double precision, parameter :: eps = 0.001D+00
  logical even
  integer i1
  integer i2
  integer ier
  ...
  logical qinr
  double precision qn(3)
  double precision qnrm
  double precision v1(3)
  double precision v2(3)
  double precision v3(3)
  double precision vn(3)
  double precision vnrm
  double precision xv(lv)
  double precision yv(lv)
  double precision zv(lv)
!
...
!
  ...
  lft2 = cn(1)*xv(i2) + cn(2)*yv(i2) + cn(3)*zv(i2) > 0.0D+00
!
...
!  <B,QN> > 0 and <B,PN> > 0.
!
    if ( dot_product ( b(1:3), qn(1:3) ) > 0.0D+00 .and. &
         dot_product ( b(1:3), pn(1:3) ) > 0.0D+00 ) then
!
...
!  Parameters:
!
!    Input, double precision P1(3), P2(3), the coordinates of unit vectors.
!
!    Input, double precision CN(3), the coordinates of a nonzero vector which defines C
...
!    should be P X Q.
!
!    Output, double precision P(3), point of intersection defined above unless IER
...
!  Parameters:
!
!    Input, double precision X1, Y1, Z1 = Coordinates of N1.
!
!    Input, double precision X2, Y2, Z2 = Coordinates of N2.
!
!    Input, double precision X0, Y0, Z0 = Coordinates of N0.
!
...
!  Parameters:
!
!    Input, double precision P(3), the Cartesian coordinates of the point P to 
...
!
!    Input, double precision X(N), Y(N), Z(N), the Cartesian coordinates of
!    the nodes.
!
!    Input, integer LIST(6*(N-2)), LPTR(6*(N-2)), LEND(N), the data structure 
!    defining the triangulation, created by TRMESH.
!
!    Output, double precision AL, the arc length between P and node NEARND.
...
!  Parameters:
!
!    Input, double precision X(*), Y(*), Z(*), the nodal coordinates.
!
...
!  Parameters:
!
!    Input, double precision PX, PY, PZ, the coordinates of P.
!
!    Output, double precision PLAT, the latitude of P in the range -PI/2 to PI/2, or
!    0 if PNRM = 0.
!
!    Output, double precision PLON, the longitude of P in the range -PI to PI, or 0
!    if P lies on the Z-axis.  
!
!    Output, double precision PNRM, the magnitude (Euclidean norm) of P.
!
  implicit none
!
...
!  Parameters:
!
!    Input, double precision X, the value to be stored.
!
!    Output, double precision STORE, the value of X after it has been stored and
...
!
!    Input, double precision RLAT(N), latitudes of the nodes in radians.
!
!    Input, double precision RLON(N), longitudes of the nodes in radians.
!
!    Output, double precision X(N), Y(N), Z(N), the coordinates in the range -1 to 1.
...
!
!    Input, double precision P(3), the x, y, and z coordinates (in that order) of the 
!    point P to be located.
!
!    Input, integer N, the number of nodes in the triangulation.  N >= 3.
!
!    Input, double precision X(N), Y(N), Z(N), the coordinates of the triangulation 
!    nodes (unit vectors).
!
!    Input, integer LIST(6*(N-2)), LPTR(6*(N-2)), LEND(N), the data structure 
!    defining the triangulation, created by TRMESH.
!
!    Output, double precision B1, B2, B3, the unnormalized barycentric coordinates of
...
!
  ...
  integer nst
  double precision p(3)
  double precision ptn1
  double precision ptn2
  double precision q(3)
  double precision s12
  double precision store
...
!
    ...
    q(2) = y(n1) - s12*y(n2)
    q(3) = z(n1) - s12*z(n2)
    if ( xp*q(1) + yp*q(2) + zp*q(3) >= 0.0D+00 ) go to 11

    if ( x(next)*q(1) + y(next)*q(2) + z(next)*q(3) >= 0.0D+00 ) go to 11
!
...
!    3 <= N <= 9999.
!
!    Input, double precision X(N), Y(N), Z(N), the coordinates of the nodes if 
...
!    Input, integer N, the number of nodes in the triangulation.  N >= 3.
!
!    Input, double precision X(N), Y(N), Z(N), the coordinates of distinct
...
!
!    Workspace, integer NEAR(N), integer NEXT(N), double precision DIST(N), 
...
!
!    Input, double precision PLTSIZ, the plot size in inches.  A circular window in
...
!
!    Input, double precision ELAT, ELON, the latitude and longitude (in degrees) of
...
!
!    Input, double precision A, the angular distance in degrees from E to the boundary 
...
!    Input, integer N, the number of nodes in the triangulation.  N >= 3.
!
!    Input, double precision X(N), Y(N), Z(N). the coordinates of the nodes (unit vectors).
!
!    Input, integer LIST(6*(N-2)), LPTR(6*(N-2)), LEND(N), the data structure 
!    defining the triangulation, created by TRMESH.
!
...
!
  if ( lun < 0 ) then
    ...
    ier = 1
    return
  else if ( pltsiz < 1.0D+00 ) then
    ier = 1
    return
  else if ( pltsiz > 8.5D+00 ) then
    ier = 1
    return
  else if ( n < 3 ) then
    ier = 1
  ...
  if ( abs ( elat ) > 90.0D+00 ) then
    ier = 2
    return
  else if ( abs ( elon ) > 180.0D+00 ) then
    ier = 2
    return
  else if ( a > 90.0D+00 ) then
    ier = 2
    return
  end if
!
...
!
      if ( z1 < 0.0D+00 .or. x1*x1 + y1*y1 > wrs .or. n1 >= n0 ) then
...
!    and N <= 9999.
!
!    Input, double precision X(N), Y(N), Z(N), the Cartesian coordinates of the nodes 
...
!    K = 1 to N.
!
!    Input, double precision XC(NT), YC(NT), ZC(NT), the coordinates of the triangle
...
!
  if (lun < 0 .or. lun > 99  .or. &
      pltsiz < 1.0D+00 .or. pltsiz > 8.5D+00  .or. &
      n < 3 .or. nt /= 2*n-4) then
    ier = 1
    return
  end if

  if ( abs ( elat ) > 90.0D+00 .or. abs ( elon ) > 180.0D+00 .or. a > 90.0D+00 ) then
    ier = 2
    return
  end if
!
...
!
    in2 = z2 >= 0.0D+00 .and. x2*x2 + y2*y2 <= wrs
!
