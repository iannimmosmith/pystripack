...
!
  ...
  write ( *, '(a,f12.6)' ) &
    '  Exact area from spherical geometry = ', 4.0E+00 * r_pi ( )
...
!
!    Input, real X(N), Y(N), Z(N), the coordinates of points 
!    on the sphere.
!
!    Output, real XC(6*(N-2)), YC(6*(N-2)), ZC(6*(N-2)), the 
...
!
!    Input, real XC(6*(N-2)), YC(6*(N-2)), ZC(6*(N-2)), the 
!    coordinates of the vertices.
!
...
!    one?" because the vertex index also tells us what polygon we are in.)
!
!    Output, real AREA_SPHERE, the area of the sphere.
