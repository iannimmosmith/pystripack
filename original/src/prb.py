...
'''
#print zip(rlon,rlat,sc*rlon,sc*rlat)
...
'''
sp.trlprt(rlon,rlat,z,iflag,ltri[:,:nt]) # ltri is possibly oversized
'''
!
...
'''
title = '(Triangulation created by STRIPACK_PRB)'
trplot_file_name = 'pystripack_prb_del.eps'
ier = sp.trplot(trplot_file_name,pltsiz,elat,elon,a,x,y,z,list,lptr,lend,title,numbr)
assert ier == 0
'''
  if ( ier == 0 ) then
    write ( *, '(a)' ) ' '
...
#print zip((x,y,z))
for kt in range(nt):
    (n1,n2,n3) = ltri[:3,kt]-1
    v1[:3] = [x[n1],y[n1],z[n1]]
    v2[:3] = [x[n2],y[n2],z[n2]]
    v3[:3] = [x[n3],y[n3],z[n3]]
#    print kt,n1,n2,n3,sp.areas ( v1, v2, v3 ),v1,v2,v3
    area = area + sp.areas ( v1, v2, v3 )
'''
  area = area / ( 16.0E+00 * atan ( 1.0E+00 ) )
'''
area = area / ( 16. * np.arctan ( 1.0 ) )
'''
  write ( *, '(a)' ) ' '
...
'''
print ' '
print '  Output from BNODES:'
print '  Number of boundary nodes = ', nb
print '  Number of arcs =           ', na
